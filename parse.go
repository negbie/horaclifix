...
func ParseRecSipUDP(msg []byte) *IPFIX {
	...
	*/
	...
	if i.SIP.VL != 6 {
		...
		i.SIP.SrcIP = net.IPv4(msg[27], msg[28], msg[29], msg[30]).To4()
		i.SIP.DstIP = net.IPv4(msg[31], msg[32], msg[33], msg[34]).To4()
...
func ParseSendSipUDP(msg []byte) *IPFIX {
	...
	if i.SIP.VL != 6 {
		...
		i.SIP.SrcIP = net.IPv4(msg[pos+15], msg[pos+16], msg[pos+17], msg[pos+18]).To4()
		i.SIP.DstIP = net.IPv4(msg[pos+19], msg[pos+20], msg[pos+21], msg[pos+22]).To4()
...
func ParseRecSipTCP(msg []byte) *IPFIX {
	...
	i.SIP.IntPort = uint8(msg[9])
	i.SIP.IntVlan = binary.BigEndian.Uint16(msg[10:12])
	i.SIP.DstIP = net.IPv4(msg[12], msg[13], msg[14], msg[15]).To4()
	i.SIP.SrcIP = net.IPv4(msg[16], msg[17], msg[18], msg[19]).To4()
