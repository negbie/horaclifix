...
class Report
{
	...
	public function doRTCPReport($timestamp, $param)
	{
		...
		for ($i = 0; $i < count($data); $i++) {
			if (!is_array($data[$i]["msg"]))
				$data[$i]["msg"] = json_decode($data[$i]["msg"]);
		}
	...
	public function doRTCPServerReport($timestamp, $param, $callids)
	{
		...
		foreach ($nodes as $node) {
			...
			$noderows                      = $db->loadObjectArray($query);
			if (SYSLOG_ENABLE == 1)
				syslog(LOG_WARNING, "RTCP Query: " . $query);
			$data = array_merge($data, $noderows);
			$limit -= count($noderows);
		}
		...
		foreach ($data as $key => $field) {
			...
			if (!array_key_exists($ipkey, $chartData))
				$chartData[$ipkey] = array();
			...
			if (array_key_exists('report_blocks', $data[$key]["msg"])) {
				...
				foreach ($blocks as $r => $block) {
					...
					if ($block["ia_jitter"] > $statsData[$ipkey]["jitter_max"])
						$statsData[$ipkey]["jitter_max"] = $block["ia_jitter"];
	...
	public function doRTCPXRServerReport($timestamp, $param, $callids)
	{
		...
		foreach ($data as $key => $field) {
			...
			if ($tmpJitter > $statsData[$ipkey]["jitter_max"])
				$statsData[$ipkey]["jitter_max"] = $tmpJitter;
			if (!array_key_exists("mos_worst", $statsData[$ipkey]) || $statsData[$ipkey]["mos_worst"] > $tmpMos)
				$statsData[$ipkey]["mos_worst"] = $tmpMos;
			$chartData[$ipkey]["mos"][] = array(
				$msts,
	...
	public function doRtpAgentReport($timestamp, $param, $callids)
	{
		...
		foreach ($data as $key => $field) {
			...
			$statsData[$ipkey]["jitter_avg"]   += $tmpJitter;
			$statsData[$ipkey]["packets_lost"] += $tmpPacketLost;
			if ($maxJitter > $statsData[$ipkey]["jitter_max"])
				$statsData[$ipkey]["jitter_max"] = $maxJitter;
			if (!array_key_exists("mos_worst", $statsData[$ipkey]) || $statsData[$ipkey]["mos_worst"] > $minMos)
				$statsData[$ipkey]["mos_worst"]  = $minMos;
			$chartData[$ipkey]["mos"][]          = array(
				$msts,
				$tmpMos
			);
	...
	public function doHoraclifixReport($timestamp, $param, $callids)
	{
		...
		foreach ($nodes as $node) {
			...
			$data                          = array_merge($data, $noderows);
			if (SYSLOG_ENABLE == 1)
				syslog(LOG_WARNING, "Horaclifix Query: " . $query);
			$limit -= count($noderows);
		}
	...
	public function getCodecsFromMessagesForTransaction($timestamp, $param)
	{
		...
		foreach ($data as $k => $d) {
			...
			if ($d["method"] == "INVITE" && $d["content_type"] == "application/sdp" && strlen($d["to_tag"]) == 0) {
				...
				if (in_array($d["cseq"], $cseq[$callid]["invcseq"]) || empty($cseq[$callid]))
					$preset[$callid]["bparty"][] = $d;
						...
						if (count($audio)) {
							...
							if (!array_key_exists($akey, $globalAudio)) {
								...
								if ($party == "aparty") {
									...
									$aIndex = $globalAudioIndex[$cid];
									if (array_key_exists($ks, $apartyAudioIndex[$cid]))
										$aIndex = $apartyAudioIndex[$cid][$ks];
						...
						if (count($video)) {
							...
							if (!array_key_exists($akey, $globalVideo)) {
								...
								if ($party == "aparty") {
									...
									$aIndex = $globalVideoIndex[$cid];
									if (array_key_exists($ks, $apartyVideoIndex[$cid]))
										$aIndex = $apartyVideoIndex[$cid][$ks];
								}
		...
		foreach ($data as $key => $row) {
			if ($row['type'] == 1 && $id != "raw") {
				...
				foreach ($drs as $k => $d) {
					...
					$a = explode(' ', $v[1]);
					/* short report */
					if ($id == "short" && !in_array($v[0], $allowreport))
						continue;
					$dval = array();
					foreach ($a as $are) {
						$b = explode('=', $are);
	...
	public function getContainer($name)
	{
		if (!$this->_instance || !isset($this->_instance[$name]) || $this->_instance[$name] === null) {
			...
			if ($name == "auth")
				...
				$containerClass = sprintf("Database\\Layer\\" . DATABASE_DRIVER);
			else if ($name == "db")
